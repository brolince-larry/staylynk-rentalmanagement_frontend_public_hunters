  ...
  return (
    <main className="min-h-screen bg-white">
      <section className="mx-auto grid max-w-[1480px] items-center gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_1fr] lg:px-8">
        <div>
          <span className="inline-flex rounded-full bg-blue-50 px-3 py-1 text-[11px] font-black uppercase text-blue-700">Our Story</span>
          <h1 className="mt-5 max-w-3xl text-4xl font-black leading-tight text-slate-950 sm:text-5xl">
            Building Trust Between Renters and <span className="text-blue-600">Landlords</span>
          </h1>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Link to="/browse" className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-5 py-3 text-sm font-black text-white">
              <Search size={15} />
              Browse Properties
            </Link>
            <Link to="/list-property" className="inline-flex items-center gap-2 rounded-lg border border-blue-600 px-5 py-3 text-sm font-black text-blue-600">
              <Home size={15} />
              List Your Property
            </Link>
          </div>
      ...
      <section className="mx-auto max-w-[1480px] px-4 pb-10 sm:px-6 lg:px-8">
        <div className="grid gap-4 rounded-lg border border-slate-200 bg-white p-6 shadow-sm sm:grid-cols-2 lg:grid-cols-4">
          {stats.map(stat => (
            <div key={stat.label} className="flex items-center justify-center gap-5 border-slate-100 lg:border-r last:border-r-0">
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-blue-50 text-blue-600">{stat.icon}</div>
              <div>
      ...
      <section className="mx-auto grid max-w-[1480px] gap-10 px-4 py-8 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8">
        <div className="relative">
          ...
          />
          <div className="absolute bottom-5 left-5 rounded-lg bg-white p-4 shadow-xl">
            <div className="flex items-center gap-3">
              <span className="flex h-12 w-12 items-center justify-center rounded-full bg-blue-50 text-blue-600"><Users size={22} /></span>
              <p className="max-w-40 text-sm font-black text-slate-950">A passionate team working for you</p>
            </div>
          </div>
        </div>
        <div>
          <span className="inline-flex rounded-full bg-blue-50 px-3 py-1 text-[11px] font-black uppercase text-blue-700">Who We Are</span>
          <h2 className="mt-5 text-3xl font-black leading-tight text-slate-950">We’re more than just a property platform.</h2>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {features.map(feature => (
              <article key={feature.title} className="flex gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-600">{feature.icon}</span>
                <div>
      ...
      <section className="mx-auto max-w-[1480px] px-4 py-8 sm:px-6 lg:px-8">
        <div className="rounded-lg bg-blue-50 p-6">
          <h2 className="text-2xl font-black text-slate-950">Our Mission, Vision & Values</h2>
      ...
      <section className="mx-auto max-w-[1480px] px-4 py-8 sm:px-6 lg:px-8">
        <h2 className="mb-5 text-center text-2xl font-black text-slate-950">Trusted by Thousands</h2>
        <div className="grid gap-5 md:grid-cols-3">
          {testimonials.map(item => (
            <article key={item.name} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              <p className="text-amber-400">★★★★★</p>
              <p className="mt-3 text-sm font-medium leading-6 text-slate-700">“{item.quote}”</p>
              <p className="mt-4 text-sm font-black text-slate-950">{item.name}</p>
      ...
      <section className="mx-auto max-w-[1480px] px-4 py-6 sm:px-6 lg:px-8">
        <div className="flex flex-wrap items-center justify-between gap-5 rounded-lg bg-blue-700 p-6 text-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-blue-50">Join thousands of satisfied renters and landlords on StayLynk today.</p>
            </div>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link to="/browse" className="rounded-lg bg-white px-5 py-3 text-sm font-black text-blue-700">Browse Properties</Link>
            <Link to="/list-property" className="rounded-lg border border-white/60 px-5 py-3 text-sm font-black text-white">List Your Property</Link>
          </div>
        </div>
      </section>
...
function Value({ icon, title, text }: { icon: React.ReactNode; title: string; text: string }) {
  return (
    <article className="flex gap-4">
      <span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-white text-blue-600">{icon}</span>
      <div>
        ...
        {title === 'Our Values' && (
          <div className="mt-3 grid gap-1 text-sm font-medium text-slate-700">
            {['Integrity', 'Transparency', 'Innovation', 'Customer Focus', 'Excellence'].map(item => (
              <span key={item} className="inline-flex items-center gap-2"><Check size={14} className="text-blue-600" />{item}</span>
            ))}
          </div>
        )}
      </div>
